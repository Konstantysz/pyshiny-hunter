...
def show_config_dialog(default_num_workers: int = 4) -> ConfigResult:
    ...
    Example:
        >>> result = show_config_dialog(default_num_workers=4)
    ...
    try:
        ...
        if window is None:
            logger.error("Failed to create GLFW window for configuration dialog")
            return ConfigResult(
        ...
        if not pokemon_db:
            logger.error("Failed to load Pokemon database - autocomplete will not work")
        else:
        ...
        while not glfw.window_should_close(window):
            ...
            if imgui.radio_button(
                "Pause - Pause all workers until manual resume", action_mode == 1
            ):
                action_mode = 1
            imgui.same_line()
            ...
            imgui.same_line()
            imgui.text(f"(1-{config.MAX_WORKERS} workers, recommended: 4)")
            ...
            if imgui.button("Start Hunting", width=-1, height=40):
                ...
                if pokemon_input.strip():
                    is_valid, validation_error, canonical_name = validate_pokemon_name(
                        pokemon_input, pokemon_db
                    )

                    if is_valid and canonical_name:
                        # Use canonical name from validation (already case-correct)
                        selected_pokemon = canonical_name

                        # Create result
                        action_str = ["alert", "pause", "continue"][action_mode]
                        result = ConfigResult(
                            target_pokemon=selected_pokemon,
